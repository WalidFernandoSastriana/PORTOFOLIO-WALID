    ...
    <section id="activities" className="py-20 bg-slate-900 text-white">
      <div className="max-w-6xl mx-auto px-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8">
          {allActivities.map((a, i) => (
            <div
              key={i}
              className="bg-slate-800 rounded-xl shadow-lg overflow-hidden hover:scale-105 transition cursor-pointer flex flex-col h-full border border-slate-700"
              onClick={() => {
                setSelected(a);
                setCurrentIndex(0);
              }}
      ...
      {selected && (
        ...
        >
          ...
          >
            <button
              className="absolute -top-12 right-0 md:top-4 md:right-4 text-white text-2xl hover:text-red-400 transition"
              onClick={() => setSelected(null)}
            >
              ✕
            </button>
            ...
            <div className="relative group">
              ...
              {selected.images.length > 1 && (
                <>
                  <button
                    onClick={handlePrev}
                    className="absolute left-2 top-1/2 -translate-y-1/2 bg-black/50 hover:bg-black/80 text-white w-10 h-10 rounded-full flex items-center justify-center transition"
                  >
                    ◀
                  </button>
                  <button
                    onClick={handleNext}
                    className="absolute right-2 top-1/2 -translate-y-1/2 bg-black/50 hover:bg-black/80 text-white w-10 h-10 rounded-full flex items-center justify-center transition"
                  >
                    ▶
                  </button>
                </>
